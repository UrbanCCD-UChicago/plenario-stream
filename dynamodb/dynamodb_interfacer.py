...
def input_defining_observation(client, network_name, observation):
    ...
    for i in range(0, len(observation['results'])):
        ...
        for j in observation.get('results').get(feature).keys():
            item[j] = {}
            if isinstance(observation.get('results').get(feature).get(j), float):
                item[j].update(serializer.serialize(Decimal(repr(observation.get('results').get(feature).get(j)))))
            else:
...
# Test setup
c = make_client()
create_network_table(c, 'TestNetwork')
input_defining_observation(c, 'TestNetwork', {'id': 'TestNetwork1',
...
input_observation(c, 'TestNetwork', {'id': 'TestNetwork1',
                                     'time': '2016-07-04T18:32:22',
                                     'results': {
                                         'temperature': {
                                             'temperature': 90.1
                                         },
