...
function recordProcessor() {
  ...
  var shardId;
  var socket = require('socket.io-client')('http://54.152.59.142:80/');
  ...
  return {
    ...
    initialize: function(initializeInput, completeCallback) {
      ...
      socket.emit('chat message', 'Did it');
    ...
    processRecords: function(processRecordsInput, completeCallback) {
      ...
      for (var i = 0 ; i < records.length ; ++i) {
        ...
        socket.emit('chat message', util.format('ShardID: %s, Record: %s, SeqenceNumber: %s, PartitionKey:%s', shardId, data, sequenceNumber, partitionKey));
